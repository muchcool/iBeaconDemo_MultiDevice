...
class ReceiveViewController: UIViewController,CLLocationManagerDelegate,UITableViewDelegate,UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        beaconRegion = CLBeaconRegion(proximityUUID: UUID(uuidString: Settings.senderUUID)!, identifier: Settings.beaconIdentifier)
    ...
    func locationManager(_ manager: CLLocationManager, didRangeBeacons beacons: [CLBeacon], in region: CLBeaconRegion) {
        ...
        if beacon.accuracy >= 0 {//如果检测不到发送端,accuracy会变成-1
            ...
            print("maybe no ibeacon sender now")
            logTxt = logTxt + "maybe no ibeacon sender now\n"
        }
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let name = Settings.deviceDic2[Int(deviceList[indexPath.row].major)]!
        cell?.textLabel?.text = "离\(name)有"+"\(String(format: "%.3f", deviceList[indexPath.row].accuracy))米"
