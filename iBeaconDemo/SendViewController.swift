...
class SendViewController: UIViewController,CBPeripheralManagerDelegate {
    ...
    override func viewDidLoad() {
        ...
        //目前只测两设备收发,暂统一用同一个uuid作为发送, 接收端也统一接收此uuid
        var major : UInt16?
        if Settings.deviceDic[Settings.beaconIdentifier] == nil {major = 1} else {major = Settings.deviceDic[Settings.beaconIdentifier]}
        beaconRegion = CLBeaconRegion(proximityUUID: UUID(uuidString: Settings.senderUUID)!, major: major!, minor: 1, identifier: Settings.beaconIdentifier)
